...
function SetJosephWestHeaderColor(colorCSSVar, shadowCSSVar) {
    const headerElement = GetElementByJSSelector("JosephWestHeader") as HTMLElement;
    headerElement.style.color = `var(${colorCSSVar})`;
    headerElement.style.textShadow = `1px 1px 5px var(${shadowCSSVar})`;
}

const projectHeader = GetElementByJSSelector("ProjectsHeader") as HTMLElement;
const skillsHeader = GetElementByJSSelector("SkillsHeader") as HTMLElement;
const contactHeader = GetElementByJSSelector("ContactHeader") as HTMLElement;
